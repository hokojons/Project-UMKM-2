class Train:
    def __init__(self, train_number, destination, seats, ticket_price):
        ...
        self.seats = seats  # Total seats available
        self.booked_seats = 0  
        self.passenger_info = {}  # Dictionary to store passenger name and seat number
        self.ticket_price = ticket_price  # Price per ticket

    def book_ticket(self, number_of_tickets, passenger_name, seat_numbers):
        if self.booked_seats + number_of_tickets <= self.seats:
            ...
            print(f"{number_of_tickets} tickets booked successfully for train {self.train_number} to {self.destination} for {passenger_name}. Total price: ${total_price}.")
        else:
...
def main():
    ...
    # Adding some trains with ticket prices
    system.add_train(Train("101", "Jakarta", 100, 50))  # Price per ticket: $50
    system.add_train(Train("102", "Bandung", 50, 30))   # Price per ticket: $30
    system.add_train(Train("103", "Yogyakarta", 75, 40)) # Price per ticket: $40
    ...
    while True:
        ...
        # Get available seats for the selected train
        ...
        while True:
            ...
            seat_numbers = [seat.strip() for seat in seat_numbers]  # Clean up whitespace
